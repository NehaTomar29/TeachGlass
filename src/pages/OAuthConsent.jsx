    ...
    return (
      <AuthLayout icon={ShieldCheck} title="Authorize access">
        <div className="flex items-center justify-center py-6 text-slate-500">
          <Loader2 className="w-5 h-5 mr-2 animate-spin text-indigo-600" aria-hidden="true" />
          Loading…
        </div>
      </AuthLayout>
    );
    ...
    >
      ...
      {tools.length > 0 && (
        <ul className="space-y-2 text-sm mb-6">
          {tools.map((tool) => (
            <li key={tool.name} className="flex flex-col border-b border-slate-100 pb-2 last:border-b-0">
